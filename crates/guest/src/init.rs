...
use nix::{
  ioctl_write_int_bad,
  unistd::{dup2, execve, fork, ForkResult, Pid},
};
...
use krata::{
  ethtool::EthtoolHandle,
  idm::client::IdmInternalClient,
  idm::internal::INTERNAL_IDM_CHANNEL,
  launchcfg::{LaunchInfo, LaunchNetwork, LaunchPackedFormat},
};

use crate::{
  background::GuestBackground,
  spawn::child::{Child, ChildSpec},
};
...
impl GuestInit {
  ...
  pub async fn init(&mut self) -> Result<()> {
    ...
    // self.supervisor.spawn_reaper().await;
  ...
  async fn run(
    ...
    if let Some(config_env) = config.env() {
      env.extend(Self::env_map(config_env));
    }
    ...
    let env = Self::env_list(env);
   
    trace!("running zone command: {}", cmd.join(" "));
   
    let path = CString::new(path.as_os_str().as_bytes())?;
    let cmd = Self::strings_as_cstrings(cmd)?;
    let env = Self::strings_as_cstrings(env)?;
    ...
    self.fork_and_exec(idm, cgroup, working_dir, path, cmd, env).await
  }
